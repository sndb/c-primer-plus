...
enum choice { A, B, C, D, E, F, Q };
const char *choices = "abcdefq";
...
{
	int f;
	printf("To choose a flight, enter its number label:\n"
	       "1) Flight 102\n"
	       "2) Flight 311\n"
	       "3) Flight 444\n"
	       "4) Flight 519\n"
	       "0) quit\n");
	while (scanf("%d", &f) != 1 || (f < 0 || f > 4)) {
		printf("You should enter the number between 0 and 4: ");
		clearinput();
	}
	clearinput();
	return f;
...
void showAlphaListSeats(struct seat seats[], size_t n)
{
	...
	for (size_t i = 0; i < n; i++)
		if (tmp[i]->assigned)
			printf("ID: %d, %s, %s, %s\n", tmp[i]->id, (tmp[i]->confirmed) ? "Y" : "X",
			       tmp[i]->lastname, tmp[i]->firstname);
...
void confirmAssignment(struct seat seats[], size_t n)
{
	int id;
	printf("Enter customer ID for confirmation: ");
	if (scanf("%d", &id) == 1 && id > 0 && (size_t)id <= n && !seats[id - 1].confirmed)
		seats[id - 1].confirmed = 1;
	clearinput();
}
...
int main(void)
{
	struct seat flights[FLIGHTS][SEATNUM] = {0};
	int f;
	enum choice i;
	void (*seatfuncs[])(struct seat *, size_t) = {showNumEmptySeats,  showListEmptySeats,
						      showAlphaListSeats, assignCustomer,
						      deleteCustomer,	  confirmAssignment};
	FILE *datafile = fopen("flights.dat", "r+b");

	// initialize datafile if not found
	if (!datafile) {
		printf("Creating and initializing datafile...\n");
		for (size_t j = 0; j < FLIGHTS; j++)
			for (size_t i = 0; i < SEATNUM; i++)
				flights[j][i].id = i + 1;
		datafile = fopen("flights.dat", "w+b");
		if (fwrite(flights, sizeof(struct seat), FLIGHTS * SEATNUM, datafile) !=
		    FLIGHTS * SEATNUM) {
	...
	if (fread(flights, sizeof(struct seat), FLIGHTS * SEATNUM, datafile) != FLIGHTS * SEATNUM) {
	...
	// main loop
	while ((f = getFlightChoice())) {
		while ((i = getFuncChoice()) != Q) {
			seatfuncs[i](flights[f - 1], SEATNUM);
		}
	}
